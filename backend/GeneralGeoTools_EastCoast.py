# env geo_env
# Tools to compute distance, and depth for ocean portfolios - East Coast
...
def GetDepthData(InputDataPath, DepthFile="Depths_EastCoast_Offshore.nc"):   

    Depth_NETCDF = xr.open_dataset(InputDataPath+"/"+DepthFile) #xr library is used here since the data is in a .nc format
    ...
    RefDepth=np.reshape(Depth_NETCDF.depth.data,-1) #The -1 here flattens the array into 1 dimension - a column of the depths with i number of rows
    RefDepth_LatLong = np.array(list(product(Lat, Long)))
    
    return RefDepth, RefDepth_LatLong    


def GetDepthCountours(InputDataPath, DepthsIn=[30, 100, 500], LatMaxMin=None):

    RefDepth, RefDepth_LatLong = GetDepthData(InputDataPath)

    # If no lat range given, derive from the data
    if LatMaxMin is None:
        LatMaxMin = (np.min(RefDepth_LatLong[:,0]), np.max(RefDepth_LatLong[:,0]))
...
def GetCoastLine_LatLong(CoastlineShpPath, LatMaxMin=None, LongMaxMin=None, PointSpacingDeg=0.01):
    """Extract coastline points from a shapefile, returning an Nx2 array of [Lat, Long].
    
    CoastlineShpPath: Full path to the coastline .shp file (e.g. ne_10m_coastline.shp)
    LatMaxMin: (min_lat, max_lat) bounding box filter. None = no filter.
    LongMaxMin: (min_lon, max_lon) bounding box filter. None = no filter.
    PointSpacingDeg: Approximate spacing (in degrees) for densifying coastline segments.
                     Smaller = more points = more accurate distance calcs but slower.
    """
    from shapely.ops import unary_union

    gdf = gpd.read_file(CoastlineShpPath)

    # Optionally clip to bounding box (with a small buffer so edges are included)
    if LatMaxMin is not None and LongMaxMin is not None:
        buf = 1.0  # degree buffer around bounding box
        bbox = (LongMaxMin[0] - buf, LatMaxMin[0] - buf, LongMaxMin[1] + buf, LatMaxMin[1] + buf)
        gdf = gdf.cx[bbox[0]:bbox[2], bbox[1]:bbox[3]]

    # Merge all geometries and densify to get evenly spaced points
    coastline_union = unary_union(gdf.geometry)

    CoastLine = []
    # Handle both single and multi-geometry cases
    geoms = coastline_union.geoms if hasattr(coastline_union, 'geoms') else [coastline_union]

    for geom in geoms:
        # Densify: interpolate points along the line at regular intervals
        length = geom.length  # in degrees (approximate)
        if length == 0:
            continue
        n_points = max(int(length / PointSpacingDeg), 2)
        for i in range(n_points + 1):
            pt = geom.interpolate(i / n_points, normalized=True)
            lat, lon = pt.y, pt.x
            # Apply bounding box filter if specified
            if LatMaxMin is not None and (lat < LatMaxMin[0] or lat > LatMaxMin[1]):
                continue
            if LongMaxMin is not None and (lon < LongMaxMin[0] or lon > LongMaxMin[1]):
                continue
            CoastLine.append([lat, lon])

    CoastLine = np.array(CoastLine)

    return CoastLine

def GetDistanceToShore(InputDataPath, LatLong, CoastlineShpPath=None, PointSpacingDeg=0.01):
    """Calculate minimum distance to shore for each point in LatLong [km].
    
    InputDataPath: Base path for data (used to find default shapefile location)
    LatLong: np.array with shape (#Points, 2) where [:,0]=Lat and [:,1]=Long
    CoastlineShpPath: Full path to coastline .shp file. If None, defaults to
                      InputDataPath + '/Geospatial Data/CoastLine/ne_10m_coastline.shp'
    PointSpacingDeg: Spacing for coastline densification (degrees)
    """
    if CoastlineShpPath is None:
        CoastlineShpPath = InputDataPath + '/Geospatial Data/CoastLine/ne_10m_coastline.shp'

    # Get coastline points filtered to the data's bounding box
    margin = 2.0
    LatMaxMin = (np.min(LatLong[:,0]) - margin, np.max(LatLong[:,0]) + margin)
    LongMaxMin = (np.min(LatLong[:,1]) - margin, np.max(LatLong[:,1]) + margin)
    
    CoastLine = GetCoastLine_LatLong(CoastlineShpPath, LatMaxMin=LatMaxMin, LongMaxMin=LongMaxMin, PointSpacingDeg=PointSpacingDeg)
    CoastLine = CoastLine * 2 * np.pi / 360
    LatLong_rad = LatLong * 2 * np.pi / 360
    
    
    MaxSize=20# Maximum number of points to calculate distance at once (reduce memory)
    NIterations=int(np.ceil(LatLong_rad.shape[0]/MaxSize))
    
    Distance=[]
    
    print("\n Calculating distance to shore for viable site locations")
    for i in tqdm(range(NIterations)):
        LatLong_i=LatLong_rad[i*MaxSize:(i+1)*MaxSize,:]
...
def PlotGeneralGeoData(LatLong, Y_Variable, GeoDataPath="./InputData/CoastLine/", ColorBarTitle=None, Title=None, SavePath=None, s=6, LatMaxMin=None, LongMaxMin=None):
    ...
    # Derive plot limits from data if not provided
    if LatMaxMin is None:
        margin = 0.5
        LatMaxMin = (np.min(LatLong[:,0]) - margin, np.max(LatLong[:,0]) + margin)
    if LongMaxMin is None:
        margin = 0.5
        LongMaxMin = (np.min(LatLong[:,1]) - margin, np.max(LatLong[:,1]) + margin)
...
def PlotGeneralGeoData_Class(LatLong, Y_Variable, GeoDataPath, Title=None, SavePath=None, s=6, LatMaxMin=None, LongMaxMin=None):
...
def PlotsWithBOEM(GeoDataPath, PathBOEMData, BOEM_ShpDir, LatLong1, Y_Variable1, LatLong2=None, Y_Variable2=None, ColorBarTitle1=None, ColorBarTitle2=None, Title=None, SavePath=None, s=6, LatMaxMin=None, LongMaxMin=None):
    ...
    kings_county_map.plot(ax=ax, color='green', edgecolor='none', alpha=0.3,legend=True, label='Wind Lease Area')

    kings_county_map = gpd.read_file(PathBOEMData+BOEM_ShpDir["Wind Planning Areas"])
    kings_county_map.plot(ax=ax, color='red', edgecolor='none', alpha=0.3, label='Wind Planning Area')
    ...
    if LatMaxMin is None:
        margin = 0.5
        LatMaxMin = (np.min(LatLong1[:,0]) - margin, np.max(LatLong1[:,0]) + margin)
    if LongMaxMin is None:
        margin = 0.5
        LongMaxMin = (np.min(LatLong1[:,1]) - margin, np.max(LatLong1[:,1]) + margin)
...
def PlotTurbineLocations(SolutionPath, Legend, PathDataUnderLayer, LegendUnderLayerColorbar, StateCountoursPath, UnderLayerVariable=["Energy_pu", "RawResource","RawResource"],
    LCOE_Target=-1, LatMaxMin=None, LongMaxMin=None, SavePath=None):
    ...
    for i, path in enumerate(SolutionPath):
        ...
        if len(data["PathKiteDesigns"])>0:
            ...
            for ic, PathKiteDesigns in enumerate(data["PathKiteDesigns"]):
                ...
                RatedPowerKiteTurbine=np.array([float(KiteData["RatedPower"])]*len(KiteData["NumberOfCellsPerSite"]))
                KiteEnergy=KiteData['Energy_pu']#Change it in the future
                ...
                IDX_Designs.append(IDX_Designs[-1]+KiteData["LatLong"][IdxInKites,:].shape[0])           
            
            for j in range(len(IDX_Designs)-1):
                
                LatLong_tmp=LatLongKite[IDX_Designs[0+j]:IDX_Designs[1+j],:][Y_Kite[IDX_Designs[0+j]:IDX_Designs[1+j]]>0,:]
                # if len(LatLong_tmp)>0:
                plt.scatter(LatLong_tmp[:,1], LatLong_tmp[:,0], marker=RangeOfRotations[j+i], c=ColorList[i+j], s=300,  alpha=1, label=Legend[i][2][j]) 

            
    # Derive plot limits from underlayer data if not provided
    if LatMaxMin is None or LongMaxMin is None:
        all_latlongs = []
        for PathData in PathDataUnderLayer:
            d = np.load(PathData, allow_pickle=True)
            all_latlongs.append(d["LatLong"])
        if len(all_latlongs) > 0:
            combined = np.concatenate(all_latlongs)
            margin = 0.5
            if LatMaxMin is None:
                LatMaxMin = (np.min(combined[:,0]) - margin, np.max(combined[:,0]) + margin)
            if LongMaxMin is None:
                LongMaxMin = (np.min(combined[:,1]) - margin, np.max(combined[:,1]) + margin)
        else:
            if LatMaxMin is None:
                LatMaxMin = (-90, 90)
            if LongMaxMin is None:
