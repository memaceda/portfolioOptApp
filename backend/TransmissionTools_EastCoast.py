...
from GeneralGeoTools_EastCoast import GetDistanceToShore, GetDepth
...
def TL_PowerParam_AC(AC_CableData, idxCable, D_SL, RatedPower_Generation, AddExtraCable=0):
    """Determine power parameters for a transmission line and admittance (AC Configuration)
    ...
    AddExtraCable: Force the addition of an extra cable to the transmission line
    """
    ...
    Y=2*np.pi*60*C*10**(-6)*LineLength*np.tanh(Gamma*LineLength/2)/(Gamma*LineLength/2)*1.j  # Admittance [1/Ω]
    ...
    if (Max_MVA**2-(Q/2)**2)<=0:
        P_LT=-1
        S_LT=-1
        NumConductors=0  # the cable cannot be used to transmit power for this system
    ...
    # Y: Admittance of one line [1/Ω]
    
    # When P_LT=S_LT=-1 and NumConductors=0 the cable cannot be used to transmit power for this system
...
def TL_Efficiency_AC(AC_CableData, idxCable, NumConductors, D_SL, RatedPower_Generation, CF=0.5):
    """Determine the efficiency of a transmission line (AC Configuration)
...
def TL_AnnualizedCost_AC(AC_CableData, idxCable, D_SL, RatedPower_Generation, AddExtraCable=0):
    """Determine the annualized cost of a transmission line (AC Configuration)
    ...
    if NumConductors!=0: 
        ...
        FCR=0.113  # Factor of capital return
        ...
        CC_AC=Cost_PC*NumConductors*LineLength + 0.221*D_SL + 4.245*10**-3*S_LT + 0.629  # Cable cost + cable landing + cable installation [M$]
        CAPEX_AC=OPPC_AC+OPC_AC+QC_AC+CC_AC  # Capital expenditures [M$]

        OPEX_AC=0.025*CAPEX_AC  # Operational Expenditures [M$/year]

        AnnualizedCost_AC=FCR*CAPEX_AC + OPEX_AC  # Annualized transmission costs [M$/year]
...
def TL_Efficiency_DC(DC_CableData, idxCable, NumConductors, D_SL, RatedPower_Generation, CF=0.5):
    ...
    # EfficiencyDC: Efficiency of the transmission line [EnergyOut/EnergyIn]
...
def TL_AnnualizedCost_DC(DC_CableData, idxCable, D_SL, RatedPower_Generation, AddExtraCable=0):
    """Determine the annualized cost of a transmission line (DC Configuration)
    ...
    CC_DC=CableCostDC*NumConductors*LineLength + 0.221*D_SL + 4.245*10**-3*S_LT + 0.629  # Cable cost + cable landing + cable installation [M$]
    CAPEX_DC=OPPC_DC+OPC_DC+CC_DC  # Capital expenditures [M$]
    
    OPEX_DC=0.025*CAPEX_DC  # Operational Expenditures [M$/year]
    
    AnnualizedCost_DC=FCR*CAPEX_DC + OPEX_DC  # Annualized transmission costs [M$/year]
...
def GetBestTransmission(InputDataPath, AC_DataPath, DC_DataPath, RatedPower_Generation, 
                        LatMaxMin=None, LongMaxMin=None, StepsPerDegree=10, 
                        DepthMinMax=(5, 2500), DistanceShoreMinMax=(10, 2000),
                        SavePath=None, FixToCopper=False):
    """Find optimal transmission configuration for each grid point.
    
    Params:
    InputDataPath: Base path for input data (depths, coastline, etc.)
    AC_DataPath: Path to AC cable parameters Excel file
    DC_DataPath: Path to DC cable parameters Excel file
    RatedPower_Generation: Rated power of the wind farm [MW]
    LatMaxMin: (min_lat, max_lat) tuple - if None, must be provided
    LongMaxMin: (min_lon, max_lon) tuple - if None, must be provided
    StepsPerDegree: Number of possible locations per degree of latitude and longitude
    DepthMinMax: (min_depth, max_depth) in meters for feasible sites
    DistanceShoreMinMax: (min_distance, max_distance) in km for feasible sites
    SavePath: Path to save output .npz file (None = don't save)
    FixToCopper: If True, only use copper cables
    """
    
    if LatMaxMin is None or LongMaxMin is None:
        raise ValueError("LatMaxMin and LongMaxMin must be provided")
    ...
    # Create Grid of Latitudes and Longitudes for the center of energy collection of the transmission system   
    ...
    print("\n Calculating distance to shore for transmission grid points")
    ShoreDistance=GetDistanceToShore(InputDataPath, TL_LatLong)

    print("\n Calculating water depth at each transmission grid point")
    Depth=GetDepth(InputDataPath, TL_LatLong)

    # Filter for feasible sites based on depth and distance constraints
    depth_min, depth_max = DepthMinMax
    dist_min, dist_max = DistanceShoreMinMax
    IdxFeasibleSites = (Depth > depth_min) & (ShoreDistance > dist_min) & (Depth <= depth_max) & (ShoreDistance <= dist_max)
    ...
    print(f"\n Feasible transmission sites: {len(TL_ShoreDistance)}")
    ...
    for i in tqdm(range(len(TL_ShoreDistance))):
        ...
        for idxCable in range(len(AC_CableData)):
            ...
            if AnnualizedCost_AC!=-1:
                # Find the cost of the transmission line taking into account the cost of energy losses priced at 83$/MWh
        ...
        for idxCable in range(len(AC_CableData)):
            ...
            if AnnualizedCost_AC!=-1:
                CostWithLoses=AnnualizedCost_AC*10**6 + 83*RatedPower_Generation*0.5*8760*(1-EfficiencyAC)
        ...
        for idxCable in range(len(DC_CableData)):
            ...
            if AnnualizedCost_DC!=-1:
                CostWithLoses=AnnualizedCost_DC*10**6 + 83*RatedPower_Generation*0.5*8760*(1-EfficiencyDC)
    ...
    LCOE_SimpleApproximation=(S_BestACost*10**6 + 83*RatedPower_Generation*0.5*8760*(1-S_Efficiency))/(RatedPower_Generation*0.5*8760)
     
    # Remove LatLong points that do not have a feasible transmission line
    FilterIdx=(S_BestCable!=-1) & (TL_Depth < depth_max)
    ...
    print(f" Final transmission sites after filtering: {len(TL_ShoreDistance)}")
    ...
    TransmissionLineParameters={
        ...
        "MaxCableCapacity": S_MaxCableCapacity
    }  # Considering only the cables, not the transformers (Transformers rated for RatedPower_Generation)
